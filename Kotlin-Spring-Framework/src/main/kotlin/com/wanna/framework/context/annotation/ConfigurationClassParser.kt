...
open class ConfigurationClassParser(
    ...
    companion object {
        ...
        private val DEFERRED_IMPORT_SELECTOR_COMPARATOR = Comparator<DeferredImportSelectorHolder> { o1, o2 ->
            AnnotationAwareOrderComparator.INSTANCE.compare(
                o1.deferredImportSelector, o2.deferredImportSelector
            )
        }
    }
    ...
    private fun processBeanMethods(configurationClass: ConfigurationClass) {
        ReflectionUtils.doWithMethods(
            ...
            { configurationClass.addBeanMethod(BeanMethod(it, configurationClass)) },
        ) { it.getAnnotation(Bean::class.java) != null }
    }

    /**
     * 处理ImportSource注解，这个注解的作用是为别的方式导入Bean提供支持，比如在注解版的IOC容器当中，去提供对XML配置文件的处理
     *
     * @see ImportSource
    ...
    private fun processImports(
        ...
        importCandidates.forEach { candidate ->
            ...
            if (ClassUtils.isAssignFrom(ImportSelector::class.java, candidate)) {
                val selector = ParserStrategyUtils.instanceClass(candidate, environment, registry) as ImportSelector
                ...
                val registrar =
                    ParserStrategyUtils.instanceClass(candidate, environment, registry) as ImportBeanDefinitionRegistrar
