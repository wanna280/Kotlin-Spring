...
open class HandlerMethodReturnValueHandlerComposite : HandlerMethodReturnValueHandler {
    ...
    fun addReturnValueHandlers(vararg returnValueHandlers: HandlerMethodReturnValueHandler): HandlerMethodReturnValueHandlerComposite {
        this.returnValueHandlers += arrayListOf(*returnValueHandlers)
        return this
    }

    fun addReturnValueHandlers(handlers: Collection<HandlerMethodReturnValueHandler>): HandlerMethodReturnValueHandlerComposite {
    ...
    private fun selectHandler(
        ...
        returnValueHandlers.forEach {
            if (it.supportsReturnType(returnType)) {
                it.handleReturnValue(returnValue, webRequest, returnType)
                return it
            }
        }
        return null
