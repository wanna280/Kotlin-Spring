...
open class Restarter(
    ...
    @Nullable private var initialUrls: Array<URL>?
) {
    ...
    /**
     * Restarter的属性信息, 因为存在有并发问题, 因此操作这个Map时需要加锁
     */
     ...
     *
     * @param name name
     * @param objectFactory ObjectFactory, 提供属性值的回调函数
    ...
    /**
     * 根据属性名name, 从Restarter当中去移除Restarter当中的一个属性
     *
     * @param name 属性名name
     ...
     * 重启整个SpringApplication
     *
     * @param failureHandler 处理重启失败的Handler
     */
    ...
    /**
     * 在关闭SpringApplication之后, 需要清除相关的缓存信息
     */
    ...
    /**
     * 构建一个[RestartClassLoader]去提供类的加载, 并去进行真正的重启工作
     *
    ...
    protected open fun doStart(): Throwable? {
        ...
        if (logger.isDebugEnabled) {
            logger.debug("正在使用主类[$mainClassName], URLs[$urls]去启动SpringApplication")
        }

        // 执行真正的relaunch重启工作...使用RestartClassLoader去加载mainClass, 并启动mainClass...
        return relaunch(classLoader)
    }

    /**
     * 使用合适的[ClassLoader](例如[RestartClassLoader]), 去完成真正的重启工作
     *
     * @param classLoader 要用来加载"main"类的[ClassLoader]
    ...
    /**
     * 从队列当中去获取到SpringApplication的线程
     *
     * @return LeakSafeThread
     */
    ...
    private inner class LeakSafeThread : Thread() {
        ...
        fun <V : Any> callAndWait(callable: Callable<V?>): V? {
            this.callable = callable
            start()
            try {
    ...
    companion object {
        ...
        /**
         * 从给定的线程的线程栈的轨迹当中, 去推断出来合适的主启动类的类名
         *
         * @param thread thread
         * @return mainClassName, 主启动类的类名
